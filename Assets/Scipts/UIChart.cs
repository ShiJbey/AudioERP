...
public class UIChart : MonoBehaviour {
	...
	public float windowSizeSeconds  = 1;
	...
	void Start () {
		...
		if (this.dataManager != null) {
			this.nSamples = (int)(windowSizeSeconds * dataManager.sampleRateHz);
		} else {
			// Default to 250Hz
			this.nSamples = (int)(windowSizeSeconds * 250);
		}
		// Fill data with all zeros
		this.data = Accord.Math.Matrix.Zeros<float> (this.nSamples, this.desiredChannels.Length);
		//print (this.GetComponent<RectTransform>().anchoredPosition3D);
		//UpdatePlot ();
	}

	// Pushes a data point on to the array and pushes out he oldest one
	public void PushDataSample(float[] sample) {
	...
	void UpdateData() {
		// Fetch the last X data points from the data manager
	}
	...
	void CorrectLineVerts() {
		...
		for (int i = 0; i < lines.Count; ++i) {
			...
			line.positionCount = this.nSamples;
